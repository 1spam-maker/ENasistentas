...
const AiTutor: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      const response = await getAiTutorResponse(input, []);
      setMessages(prev => [...prev, { role: 'model', text: response || 'Atsiprašau, įvyko klaida.', timestamp: Date.now() }]);
    } catch (error) {
      console.error(error);
      setMessages(prev => [...prev, { role: 'model', text: 'Nepavyko susisiekti su AI mokytoju. Patikrinkite interneto ryšį.', timestamp: Date.now() }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto h-[calc(100vh-12rem)] flex flex-col bg-slate-900 rounded-[2rem] border border-slate-800 overflow-hidden shadow-2xl">
      <div className="bg-indigo-600 p-5 text-white flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-indigo-100 font-medium opacity-80 mt-1">Visada pasiruošęs padėti</p>
          </div>
        </div>
        ...
        {isTyping && (
          <div className="flex justify-start">
            <div className="bg-slate-800 border border-slate-700 p-4 rounded-2xl rounded-tl-none shadow-sm flex items-center gap-3">
              ...
              <span className="text-sm font-bold text-slate-400">Asistentas mąsto...</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-5 border-t border-slate-800 bg-slate-900">
        <div className="flex gap-3">
          <input 
            ...
            placeholder="Klauskite apie gramatiką arba tiesiog pasisveikinkite..."
            className="flex-1 p-4 bg-slate-950 border border-slate-700 rounded-2xl focus:ring-2 focus:ring-indigo-500 focus:outline-none text-[15px] text-white placeholder-slate-600"
          />
          <button 
            onClick={handleSend}
            disabled={!input.trim() || isTyping}
            className="bg-indigo-600 text-white p-4 rounded-2xl hover:bg-indigo-500 transition-all disabled:opacity-30 shadow-lg shadow-indigo-950/20"
          >
            <Send size={20} />
          </button>
        </div>
        <div className="mt-4 flex items-center gap-2 text-[11px] font-bold text-slate-500 uppercase tracking-wide">
          ...
          <span>Pavyzdys: "Kaip taisyklingai naudoti Past Simple?"</span>
        </div>
      </div>
    </div>
  );
